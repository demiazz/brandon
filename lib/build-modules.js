...
const { transform, removeTypes } = require("./transform");
...
function transformSource({ file, code }, extension, transformOptions) {
  return {
    file: file.replace(/\.js$/, `.${extension}.flow`),
    code: transform(code, transformOptions),
    flow: true
  };
}

function removeTypesFromSource({ file, code }) {
  return {
    file: file.replace(/\.flow/, ""),
    code: removeTypes(code),
    flow: false
  };
}

function transformSources(sources, modules) {
  const extension = modules === "es" ? "mjs" : "js";
  const transformOptions = {
    types: true,
    modules: modules === "es" ? false : "commonjs"
  };
  const sourcesWithTypes = sources.map(source =>
    transformSource(source, extension, transformOptions)
  );
  const sourcesWithoutTypes = sourcesWithTypes.map(removeTypesFromSource);

  return sourcesWithTypes.concat(sourcesWithoutTypes);
}

function formatSource({ file, code, flow }) {
  return {
    file,
    code: format(code),
    flow
...
function writeSources(sources, modules) {
  ...
  return Promise.all(
    sources.map(async source => writeSource(source, outputRoot))
  );
}
