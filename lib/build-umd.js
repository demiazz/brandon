...
const { name: pkgName } = require("./pkg");
...
function createWriteOptions() {
  const file = `dist/${pkgName}.js`;
  const format = "umd";
  const name = camelCase(pkgName);
  const amd = { id: pkgName };
...
async function buildMinifiedUMDBundle() {
  ...
  const source = await readFile(`dist/${pkgName}.js`);
  logger.stepDone();
  ...
  const bundle = node.toStringWithSourceMap({ file: `${pkgName}.min.js` });
  const code = `${bundle.code}\n//# sourceMappingURL=${pkgName}.min.js.map`;
  logger.stepDone();

  logger.step("Write minified module");
  await writeFile(`dist/${pkgName}.min.js`, code);
  await writeFile(`dist/${pkgName}.min.js.map`, bundle.map);
  logger.stepDone();
}
